...
export function findQuestions(mode: 'typing-practice' | 'word-quiz') {
  if (mode === 'typing-practice') {
    const selectedEasyQuestions = shuffle(practiceFindEasyQuestions()).slice(0, 6);
    const selectedMiddleQuestions = shuffle(practiceFindMiddleQuestions()).slice(0, 3);
    const selectedHardQuestions = shuffle(practiceFindHardQuestions()).slice(0, 4);
    return { selectedEasyQuestions, selectedMiddleQuestions, selectedHardQuestions }
  } else {
    const selectedEasyQuestions = shuffle(quizFindEasyQuestions()).slice(0, 6);
    const selectedMiddleQuestions = shuffle(quizFindMiddleQuestions()).slice(0, 3);
